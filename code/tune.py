...
def objective(trial):
    ...
    default_settings['batch_size'] = trial.suggest_int("batchsize", 8, 64, step=4, log=False)
    if "max_seq_len" in default_settings.keys():
        default_settings['max_seq_len'] =  trial.suggest_int("max_seq_len", 4, 40, log=False)
    ...
    if model_name == 'lgcn':
        ...
        default_settings['drop_out'] =  trial.suggest_float("drop_out", 0.0, 1.0, log=False)

    # Create model
    model = create_model(data, default_settings, silence=True)
    # Run model
    return run_model(dataloader, default_settings, model, save_settings, tune= True, silence=True)

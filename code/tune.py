...
def objective(trial):
    ...
    if model_name == "lgbm":
        default_settings["num_boost_round"] = trial.suggest_int(
            "num_boost_round", 100, 2000, step=500, log=False
        )
        # default_settings["params"]["learning_rate"] = trial.suggest_float("learning_rate", 0.001, 0.5, step= 0.001, log=False)
        default_settings["early_stopping"] = trial.suggest_int(
            "early_stopping", 10, 500, step=100, log=False
        )
